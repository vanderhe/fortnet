...
module dftbp_dynneighlist
  ...
  type :: TNeighIterator
    ...
    !> Whether to include self as neighbor
    logical :: tIncludeSelf
...
contains
  ...
  subroutine TNeighIterator_init(this, neighList, iAtom, includeSelf)
    ...
    if (present(includeSelf)) then
      this%tIncludeSelf = includeSelf
    else
      this%tIncludeSelf = .false.
    end if
  ...
  subroutine TNeighIterator_getNextNeighbours(this, nNeighbourSK, coords, dists, img2CentCell,&
    ...
    do while (nNeighbourSK < maxNeighs)
      iAtom2 = modulo(this%iAtom2 - 1, this%nAtom) + 1
      if (this%isCentral .and. iAtom2 == this%iAtom1 .and. (.not. this%tIncludeSelf) .and. (.not.&
          & this%nAtom == 1)) then
        this%iAtom2 = this%iAtom2 + 1
        this%iIter = this%iIter + 1
        cycle
      end if
      if (this%iIter > this%nAtom) then
        if (this%tPeriodic) then
