...
!> Routines to make socket contact with an external code and communicate data back and forward from
!! Fortnet to the external code.
module fnet_ipisocket
  ...
  use fnet_fsockets, only : writebuffer, readbuffer, close_socket, connect_inet_socket,&
      & connect_unix_socket
  ...
  type(IpiProtocolsEnum), parameter :: IPI_PROTOCOLS = IpiProtocolsEnum(1)
...
contains
  ...
  !> Construct an IpiSocketComm instance.
  subroutine IpiSocketComm_init(this, input)
    ...
    !! True, if local UNIX socket is desired
    logical :: tUnix

    !!
    character(len=lc) :: msg
  ...
  !> Constructs an IpiSocketComm instance.
